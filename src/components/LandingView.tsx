...
export function LandingView({ onAnalyze, activeTable, setActiveTable }: LandingViewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center px-6 py-12 overflow-y-auto relative h-full min-h-full scroll-smooth">
      ...
      <div className="w-full flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto relative z-10">
        
        <div className="mt-8 mb-6 text-center space-y-3">
          <h1 className="text-white text-4xl md:text-5xl font-extrabold leading-tight tracking-tight drop-shadow-2xl">
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          ...
          <div className="relative group cursor-pointer h-full">
            ...
            <div className="relative flex flex-col items-center justify-center border border-white/10 border-dashed rounded-3xl bg-[#0f1117]/80 backdrop-blur-2xl p-8 text-center hover:bg-white/[0.02] transition-all h-full min-h-[220px]">
              ...
              <h3 className="text-white text-lg font-bold mb-1">Import new data</h3>
              <p className="text-white/40 text-[10px] mb-4 uppercase tracking-[0.1em] font-bold">CSV supported</p>
              
              <label className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg backdrop-blur-md cursor-pointer active:scale-95">
                {isUploading ? 'Uploading...' : 'Choose File'}
                <input type="file" accept=".csv" className="hidden" onChange={handleFileUpload} disabled={isUploading} />
              </label>
              
              {uploadMessage && (
                <p className={cn("text-[10px] font-bold mt-3 uppercase tracking-wider", uploadMessage.startsWith('Error') ? "text-rose-400" : "text-emerald-400")}>
                  {uploadMessage}
                </p>
              )}
            </div>
          ...
          <div className="flex flex-col justify-center items-center p-8 rounded-3xl bg-white/[0.02] border border-white/10 backdrop-blur-3xl text-center h-full min-h-[220px]">
            <div className="w-12 h-12 bg-emerald-500/10 rounded-full flex items-center justify-center mb-4 border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.1)]">
              <Database className="w-6 h-6 text-emerald-400" />
            </div>
            <h3 className="text-white/40 text-[10px] font-bold uppercase tracking-[0.2em] mb-2">Currently Connected</h3>
            <div className="bg-emerald-500/5 px-4 py-3 rounded-2xl border border-emerald-500/10 mb-2 max-w-full">
              <p className="text-white font-mono text-sm break-all font-bold tracking-tight">
                {activeTable || 'nexus_bi_default'}
              </p>
            </div>
            <p className="text-[10px] text-emerald-400/60 font-medium uppercase tracking-widest">Active Table ready for analysis</p>
          </div>
        </div>

        {/* Query Input Section */}
        <div className="w-full max-w-3xl mb-12 relative z-20">
          <div className="relative flex flex-col md:flex-row items-stretch gap-2 w-full p-2 bg-white/[0.03] backdrop-blur-3xl rounded-2xl shadow-2xl border border-white/10 transition-all focus-within:border-indigo-500/50 focus-within:shadow-indigo-500/20">
            <div className="flex flex-1 items-center px-4 bg-white/5 rounded-xl border border-white/5 transition-colors focus-within:bg-white/10">
              <Search className="text-white/40 mr-3 w-5 h-5 shrink-0" />
              <input
                type="text"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleAnalyze()}
                className="w-full bg-transparent border-none focus:ring-0 text-white placeholder:text-white/30 text-base py-3 outline-none"
                placeholder="Ask a question about your data..."
              />
              <VoiceInput onResult={(text) => setQuery(text)} className="mr-1" />
            </div>
            <button
              onClick={handleAnalyze}
              disabled={!query.trim()}
              className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-600/50 disabled:text-white/50 text-white font-bold px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20 active:scale-95"
            >
              <span>Analyze</span>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {suggestions.map((suggestion, idx) => (
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 backdrop-blur-md hover:bg-white/10 border border-white/5 transition-all text-white/50 hover:text-white group"
              >
                <suggestion.icon className="w-3.5 h-3.5 text-white/30 group-hover:text-indigo-400 transition-colors" />
                <span className="text-[10px] font-bold uppercase tracking-wider">{suggestion.text}</span>
              </button>
            ))}
          </div>
        </div>
